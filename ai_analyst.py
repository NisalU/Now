...
Pipeline (no regime or trade-quality gates — every bar gets a full AI read):
    ...
    Market data (engine.py)
        ...
        -> Primary AI analyst (Groq, SYSTEM_PROMPT)
...
# Models that follow strict JSON mode more reliably with the shorter,
# schema-first SYSTEM_PROMPT_ENHANCED than with the full SYSTEM_PROMPT.
ENHANCED_PROMPT_MODELS = {"openai/gpt-oss-120b"}
...
def _prompt_for_model(system_prompt, model):
    """Swap in the shorter, schema-first SYSTEM_PROMPT_ENHANCED for models
    that follow strict JSON mode less reliably with the longer prompt.
    Only applies to the primary analyst prompt — the critic prompt is
    left as-is for every model."""
    if system_prompt is SYSTEM_PROMPT and model in ENHANCED_PROMPT_MODELS:
...
class AIAnalyst:
    ...
    def _call_groq_models(self, system_prompt, payload_text):
        ...
        last_exc = None
        for model in models:
            prompt = _prompt_for_model(system_prompt, model)
            try:
